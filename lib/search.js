...
const { loadWords, normalizeAlphaWord, normalizeWords } = require("./words");
...
function inferModeFromTld(tld, fallbackMode = "exact") {
  if (DEFAULT_HACK_TLDS.includes(tld)) return "hack";
...
function normalizeProvidedCandidate(candidate, fallbackMode = "exact") {
  if (typeof candidate === "string") {
    ...
    const inferredMode = inferModeFromTld(tld, fallbackMode);
    return {
      mode: inferredMode,
      input: trimmed,
      word: label,
  ...
  const resolvedMode =
    ...
    (explicitDomainShape === "creative_suffix"
      ...
      : explicitDomainShape === "exact"
        ...
        : domain
          ? inferModeFromTld(tld, fallbackMode)
          : fallbackMode);
...
async function evaluateCandidates(options = {}) {
  ...
  const availableGoal =
    options.availableGoal === undefined || options.availableGoal === null
      ? Number.POSITIVE_INFINITY
      : Number(options.availableGoal);
  ...
  function shouldStop() {
    return checked >= maxChecks || seed.available + available >= availableGoal;
  }
