...
function isWholeWordHack(candidate, options = {}) {
  const label = normalizeAlphaWord(candidate.label);
  const tld = normalizeAlphaWord(candidate.tld);
  ...
  if (tld === "in") {
    const ingWord = `${joinedWord}g`;
    if (isKnownWord(ingWord, options.sourceWordSet)) return true;
  }
...
function isCandidateLabelAllowed(candidate, options = {}) {
  ...
  if (candidate.domain_shape === "creative_suffix") {
    ...
    return isWholeWordHack(candidate, options);
  }
...
function generateHackCandidates(words, options = {}) {
  ...
  const bestByDomain = new Map();
  const sourceWordSet = options.sourceWordSet ||
    (options.trustSourceWordsForHackValidation ? buildWordSet(words) : null);
  ...
  for (const word of words) {
    for (const tld of tlds) {
      ...
      if (tld === "in" && word.endsWith("ing")) {
        const label = word.slice(0, -3);
        variations.push({
          word,
          joined_word: `${label}in`,
          label,
          tld,
          source_word: word,
        });
      ...
      for (const variation of variations) {
        const domain = `${variation.label}.${tld}`;
        if (variation.label.length < minLabelLength) continue;
